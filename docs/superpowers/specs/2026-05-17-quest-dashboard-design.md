...
```
┌─────────────────────────────────────────────────────────┐
│  ● Quest Dashboard          총 12명  완료 3명  클리어 47  │
├─────────────────────────────────────────────────────────┤
│  [카드] [카드] [카드] [카드]                              │
│  [카드] [카드] [카드] [카드]                              │
│  ...                                                    │
...
**퀘스트 칸 색상:**
- 초록 (`#4ade80`) — `purchasedSteps`에 포함된 스텝 (결제 완료 = 퀘스트 접근 시작)
- 노랑 깜박 — 가장 높은 purchasedStep이면서 `isCompleted: false` (현재 진행중으로 간주)
- 어두움 (`#2a4030`) — 미시작
- 파랑 테두리 — 신규 등록 (5분 이내 `registeredAt`)

**뱃지:**
- `완료!` (초록) — `isCompleted: true`
- `진행중` (노랑) — 1개 이상 purchasedSteps
- `신규` (파랑) — purchasedSteps 없고 5분 이내 등록
