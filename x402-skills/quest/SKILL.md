...
2. 퀘스트 접근 →  GET /v1/quest/product-a/{step}
               퀘스트 1(무료): 바로 응답
               퀘스트 2~10(1 TONE): 402 → X-PAYMENT 헤더로 재요청 → questUrl 수령
...
응답:
```json
{ "id": "quest-1", "name": "퀘스트 1 — 드래그앤드롭", "questType": "drag-drop" }
```

퀘스트 1은 무료이므로 결제 없이 바로 접근합니다.
응답의 `questType`을 확인하고, 사용자에게 웹 앱 접속을 안내하세요.
...
5. TONE 잔액 부족 시 사용자에게 알림 (초기 10 TONE, 퀘스트 2~10 각 1 TONE)
